...
namespace MUAWeb{
    ...
    void MySQLParameter::addParameter(std::string paraname,std::string content){
        std::string temp;
        temp = Utility::replace(content,"'","''");
        this->parametes.insert(std::pair<std::string,std::string>(paraname,temp));
    }
    void MySQLParameter::removeParameter(std::string parname){
        auto iter = parametes.find(parname);
        if (iter != parametes.end())
            parametes.erase(iter);
    }
    std::string MySQLParameter::getSQL(){
        for (std::pair<const std::basic_string<char>, std::basic_string<char>> para:parametes) {
